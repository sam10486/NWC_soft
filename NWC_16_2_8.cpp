...
int main(){
    ...
    vector<vector<long long > > stage0_data_in(radix_2, vector<long long > (radix_8, 0));
    vector<vector<long long > > stage0_data_fft(radix_2, vector<long long > (radix_8, 0));

    vector<vector<long long > > stage1_data_in(8, vector<long long > (radix_2, 0));
    vector<vector<long long > > stage1_data_fft(8, vector<long long > (radix_2, 0));
    ...
    for(int i=0; i<2; i++){
        for(int j=0; j<8; j++){
            stage0_data_fft[i] = NWC_forward_DIT(stage0_data_in[i], radix_8, phi_8, modular);
    ...
    for(int i=0; i<8; i++){
        cout << "-----------------------------" << endl;
        for(int j=0; j<2; j++){
            long long phi_16_dg2 = ExpMod(phi_16, 2, modular);
            long long phi_16_degree = ExpMod(phi_16_dg2, i, modular);
            stage1_data_in[i][j] = MulMod(stage1_data_in[i][j], ExpMod(phi_16_degree, j, modular), modular);
            cout << "phi_16_degree = " << phi_16_degree << endl;
    ...
    for(int i=0; i<8; i++){
        cout << "-----------------------------" << endl;
        for(int j=0; j<2; j++){
            stage1_data_fft[i] = NWC_forward_DIT(stage1_data_in[i], radix_2, phi_16, modular);
    ...
    for(int i=0; i<8; i++){
        for(int j=0; j<2; j++){
            NWC_flatten[i+j*8] =  stage1_data_fft[i][j];
            cout << "NWC_flatten[" << i+j*8 << "]" << " = " << NWC_flatten[i+j*8] << endl;
        }
    }
    ...
    BitOperate BR;
    for(int i=0; i<m; i++){
        long long idx_BR = BR.BitReserve(i, 4);
    ...
    NWC_ans = NWC(input_vector, m, phi_16, modular);
