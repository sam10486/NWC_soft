...
int main(){
    ...
    long long FFT_data_BU[radix_16][radix_8];
    long long FFT_out_BU[radix_16][radix_8];

    long long FFT_stage1_input_BU[radix_8][radix_16];
    long long FFT_stage1_output_BU[radix_8][radix_16];

    
    long long FFT_out_final[m];
    
	long long prou_128 = find_prou(m, modular);
    long long prou_16 = ExpMod(prou_128, 8, modular);
    long long prou_8 = ExpMod(prou_128, 16, modular);
	

    cout << "prou_128 = " << prou_128 << endl;
    ...
    cout << "-------------------------------" << endl;
    for(int i=0; i<16; i++){
        FFT_no_bit_reverse(FFT_out_BU[i], FFT_data_BU[i], radix_8, prou_8, modular);
        for(int j=0; j<8; j++){
    ...
    for(int i=0; i<radix_16; i++){
       for(int j=0; j<radix_8; j++){
            long long index_rev = BU.BitReserve(j, 3); //how much index numbers in one BU, and reverse the index
            long long exp_prou128 = i*index_rev;
            FFT_out_BU[i][j] = MulMod(FFT_out_BU[i][j] ,ExpMod(prou_128, exp_prou128, modular), modular);
            cout << "FFT_out_BU[" << i << "][" << j << "]" << " = " << FFT_out_BU[i][j] << endl;
        }
    }
    cout << "-------------------------------" << endl;
    ...
    for(int i=0; i<m;i++){
        long long index_rev = BU.BitReserve(i, 7);
    ...
    long long DFT_ans[m];
    DFT(DFT_ans, FFT_data_in, m, prou_128, modular);
    cout << "-------DFT---------------" << endl;
    for(int i=0; i<m; i++)
        cout << DFT_ans[i] << endl;
