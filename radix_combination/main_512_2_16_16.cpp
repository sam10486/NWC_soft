...
int main(){
    ...
    long long FFT_data_BU[256][radix_2];
    long long FFT_out_BU[256][radix_2];
    ...
    for(int i=0; i<256; i++){
        for(int j=0; j<2; j++){
            FFT_data_BU[i][j] = FFT_data_in[i+256*j];
    ...
    for(int i=0; i<256; i++){
       for(int j=0; j<radix_2; j++){
            ...
            FFT_out_BU[i][j] = MulMod(FFT_out_BU[i][j] ,ExpMod(prou_512, exp_prou512, modular), modular);
            cout << "FFT_out_BU[" << i << "][" << j << "]" << " = " << FFT_out_BU[i][j] << endl;
        }
    }
    ...
    for(int i=0; i<2; i++){
        for(int j=0; j<256; j++){
            FFT_stage1_input_BU_tmp[i*256+j] = FFT_out_BU[j][i];
            cout << "FFT_stage1_input_BU_tmp[" << i*256+j << "]" << " = " << FFT_stage1_input_BU_tmp[i*256+j] << endl;
        }
    }
    ...
    for(int i=0; i<16; i++){
        cout << "--------------------------" << endl;
        for(int j=0; j<16; j++){
            FFT_stage1_input_BU[i][j] = FFT_stage1_input_BU_tmp[i+16*j];
            FFT_stage1_input_BU[i+16][j] = FFT_stage1_input_BU_tmp[i+16*j+256];
            cout << "FFT_stage1_input_BU[" << i << "][" << j << "]" << " = " << FFT_stage1_input_BU[i][j] << endl;
            //cout << "FFT_stage1_input_BU[" << i+16 << "][" << j << "]" << " = " << FFT_stage1_input_BU[i+16][j] << endl;
        }
    }
    cout << "-------------------------------" << endl;
    for(int i=0; i<32; i++){
    ...
    for(int i=0; i<16; i++){
       ...
       for(int j=0; j<radix_16; j++){
            ...
            long long exp_prou512;
            exp_prou512 = 2*i*index_rev;
            FFT_stage1_output_BU[i][j] = MulMod(FFT_stage1_output_BU[i][j] ,ExpMod(prou_512, exp_prou512, modular), modular);
            FFT_stage1_output_BU[i+16][j] = MulMod(FFT_stage1_output_BU[i+16][j] ,ExpMod(prou_512, exp_prou512, modular), modular);
            cout << "FFT_stage1_output_BU_com[" << i << "][" << j << "]" << " = " << FFT_stage1_output_BU[i][j] << endl;
            //cout << "FFT_stage1_output_BU_com[" << i+16 << "][" << j << "]" << " = " << FFT_stage1_output_BU[i+16][j] << endl;
        }
    }
    ...
    for(int i=0; i<16; i++){
        ...
        for(int j=0; j<16; j++){
            ...
            //cout << "FFT_stage2_input_BU[" << i+16 << "][" << j << "]" << " = " << FFT_stage2_input_BU[i+16][j] << endl;
        }
    }
    cout << "-------------------------------" << endl;
    for(int i=0; i<32; i++){
