...
int main(){
    ...
    cout << "-------------stage 3 input-------------" << endl;
       for(int i=0; i<16; i++){
            cout << "-------------------------------" << endl;
            for(int j=0; j<16; j++){
                FFT_stage3_input_BU[i][j] = FFT_stage2_output_BU[j*16][i];
                FFT_stage3_input_BU[i+16*1][j] = FFT_stage2_output_BU[j*16][i];
                //FFT_stage3_input_BU[i+16][j] = FFT_stage2_output_BU[j+16][i];
                //cout << "FFT_stage3_input_BU[" << i << "][" << j << "]" << " = " << FFT_stage3_input_BU[i][j] << endl;
                cout << "FFT_stage3_input_BU[" << i+16 << "][" << j << "]" << " = " << FFT_stage3_input_BU[i+16][j] << endl;
            }
        } 
